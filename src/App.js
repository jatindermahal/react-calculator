...
const calcOutput = (state,action)=>{
  switch (action.type) {
    case "add-num":
      ...
      if((action.data.digit === "0" || action.data.digit === "00")  && (state.currentInput === "0" || state.currentInput === "00"))
        return state;

      if(state.currentInput == null && action.data.digit ==="00")
        return state;

      if(action.data.digit === "." && (state.currentInput == null || state.currentInput.includes(".")))
        return state;
...
function App() {
  ...
  const [{ currentInput, previousInput, operation}, dispatch] = useReducer(calcOutput,{})
    ...
    <>
      ...
      <div className="calculator">
        <div className="result">
          <div className="history">{previousInput} {operation}</div>
          <div className="number">{currentInput}</div>
        </div>
        {/* send type to dispatch to tell on what we need to do if button pressed */}
        <button onClick={()=>{dispatch({type:"all-clear"})}}>AC</button>
        <button onClick={()=>{dispatch({type:"clear-line"})}}>CE</button>
